...
questions about incidents and executes tool actions such as database queries and
...
itself. This document specifies a redesign of the harness around
that LLM, aimed at making it reliable and safe to operate at the company's
current incident volume.

## 2. Problem

1. A Support Engineer gave the LLM instructions for a Customer Escalation, 
   and the LLM chose to delete the database. There was no permission boundary 
   distinguishing an action the LLM may take on its own from one that needs human approval.
2. Users report the LLM is not learning, probably because it gives answers built
   on outdated data, answers the model does not actually know but invents
   anyway, and answers that are simply wrong.
3. A known, recurring set of common questions gets a different answer 
   from the LLM depending on the day it is asked, even though the facts have not changed.
4. During the first week of every month the LLM tends to not respond, or responds incorrectly, 
   more often than during normal weeks.
5. Support reports that Genius sometimes shows the status of an incident that was already closed hours earlier.


## 3. Objectives

1. Every tool action the LLM can take is classified as autonomous or
   approval-required. — Addresses Problem 1 (Support Engineer, Incident
   Manager, Data Science Lead).
2. Any status Genius returns reflects an incident's current state. —
   Addresses Problem 5 (Support Engineer, Customer).
3. The known set of common questions returns the same answer on repeated
   asking. — Addresses Problem 3 (Support Engineer, Platform Engineer).
4. Genius stays available and within its stated latency and SLA targets
   during the highest-incident-volume week of the month. — Addresses
   Problem 4 (Support Engineer, Platform Engineer, Incident Manager).
5. Every answer is traceable to whether it came from fresh data, a
   deterministic rule, or the model itself, including the model saying it
   does not know rather than inventing an answer. — Addresses Problem 2
   (Platform Engineer, Data Science Lead).
6. Every tool action the LLM executes is logged with who triggered it, what
   it did, and under which permission, so it can be reviewed after the
   fact. — Addresses Problem 1 (Platform Engineer, Incident Manager, Data
   Science Lead).

## 4. Out of Scope

- Replacing the local LLM with an external/hosted model. — Why: incident
  data can't leave the company's own infrastructure.
- A general-purpose ticketing UI redesign. — Why: this project only fixes
  the LLM and its harness, not the ticketing system around it.
- New incident categories. — Why: the three existing Escalation types
  already cover every incident Genius handles today.
- A full self-service Customer support portal. — Why: Customers only need
  to check status, not manage or act on incidents themselves.
...
- **Harness**: every component around the LLM that gives it better performance, 
...
- **Recurring-question set**: the known, bounded set of common questions
  that Support and Engineering ask about incidents daily.
...
- **Load prioritization**: the peak-week behavior of rejecting
  or deprioritizing lower-urgency requests once capacity is exceeded,
  instead of letting every request degrade silently.
- **Cache answer path**: the deterministic path that answers a question in
  the recurring-question set directly, without asking the LLM, so the same
  question always returns the same answer regardless of when it is asked.

## 6. Users and Their Needs

| Persona | Details |
|---|---|
| Support Engineer | [Users/SupportEngineer.md](Users/SupportEngineer.md) |
| Platform Engineer | [Users/PlatformEngineer.md](Users/PlatformEngineer.md) |
| Data Science Lead | [Users/DataScienceLead.md](Users/DataScienceLead.md) |
| Customer (external) | [Users/Customer.md](Users/Customer.md) |
| Incident Manager | [Users/IncidentManager.md](Users/IncidentManager.md) |
...
2. Route the known set of recurring common questions through a
   cache answer path, instead of always asking the LLM.
...
4. **LLM-assisted investigation**
   ...
   4. If the question matches the known recurring-question set, Genius
      answers through the cache answer path; otherwise the LLM answers,
...
7. **Peak-week operation**
   ...
   3. Incident Manager monitors load and SLA compliance throughout the
      week; Data Science Lead monitors whether the model and harness hold
      up under the load.
   4. Incident Manager reports reliability and SLA-compliance numbers to
      the business.
